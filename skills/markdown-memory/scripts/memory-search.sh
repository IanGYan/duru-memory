...
# Deterministic markdown memory search (no embeddings)
...
SEMANTIC_MIN_SCORE="${SEMANTIC_MIN_SCORE:-0.48}"
FUSION_MODE="${FUSION_MODE:-rrf}"          # rrf | linear
RRF_K="${RRF_K:-60}"
KEYWORD_BOOST="${KEYWORD_BOOST:-0.006}"   # extra boost for strong keyword matches in RRF mode
PRIOR_BOOST="${PRIOR_BOOST:-0.004}"       # directory prior boost
FUSION_SEM_WEIGHT="${FUSION_SEM_WEIGHT:-0.65}"
FUSION_KEY_WEIGHT="${FUSION_KEY_WEIGHT:-0.35}"
...
if [[ -x "$SEMANTIC_SCRIPT" ]]; then
  ...
  if semantic_json="$(python3 "$SEMANTIC_SCRIPT" "$QUERY" "$WORKSPACE" --min-score "$SEMANTIC_MIN_SCORE" 2>/dev/null)"; then
