...
# Session startup helper for markdown-memory skill
...
printf '=== markdown-memory: session start ===\n'
printf 'workspace: %s\n' "$WORKSPACE"
printf 'timestamp: %s\n\n' "$(date '+%Y-%m-%d %H:%M:%S %z')"
...
printf '  - run scripts/memory-search.sh "<query>" before context-heavy answers\n'

printf '\n[4/4] semantic index warmup (daily, incremental)\n'
SEM_SCRIPT="${WORKSPACE}/skills/markdown-memory/scripts/memory-semantic-search.py"
STAMP_FILE="${MEMORY_DIR}/.semantic-warmup-date"
TODAY="$(date '+%Y-%m-%d')"
LAST="$(cat "$STAMP_FILE" 2>/dev/null || true)"
if [[ -x "$SEM_SCRIPT" ]]; then
  if [[ "$LAST" != "$TODAY" ]]; then
    if python3 "$SEM_SCRIPT" "warmup" "$WORKSPACE" --build-only >/dev/null 2>&1; then
